...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleInvalidReference(
            InvalidBookingReferenceException ex,
            HttpServletRequest request
    ) {
        return build(HttpStatus.BAD_REQUEST, ex.getMessage(), request);
    }
    ...
    /**
     * The circuit breaker is open: this dependency has been failing, so
     * the call was refused without being attempted.
     *
     * 503 rather than the 502 a live failure gets — 502 says the
     * dependency answered badly, 503 says we did not even ask and the
     * caller should retry later. Retry-After is not set because the
     * breaker probes on its own schedule.
     */
    ...
    public ResponseEntity<ErrorResponse> handleCircuitOpen(
        ...
        return build(
                ...
                "Dependency temporarily unavailable: " + ex.getMessage(),
                request
        );
    }
